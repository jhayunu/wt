...
import { parsePrimaryKey, rowDelta } from "./sqlrows.js";
...
async function dumpSchema(ctx: Ctx): Promise<string> {
  // wt_changesets is wt's own bookkeeping, created by `wt db apply`. Without this it shows
  // up as a schema change the agent never made, and gets exported into the next changeset.
  const r = await ddev.exec(ctx.run, ctx.rec.path, ["mysqldump", "--no-data", "--skip-comments", "--skip-triggers", "--compact", `--ignore-table=db.${LEDGER_TABLE}`, "db"]);
 ...
 * Kept as a thin alias so callers need not care whether a primary key was available.
 */
export function statementDelta(before: string[], after: string[], pk?: string[]) {
  return rowDelta(before, after, pk);
}
...
async function deltaFor(ctx: Ctx, dir: string, table: string, schema?: string) {
  ...
  // The primary key turns "old row gone + new row appeared" back into "one row changed".
  const pk = parsePrimaryKey(schema ?? await dumpSchema(ctx), table);
  return rowDelta(before, after, pk);
}
...
export const snapshotDiff: DbChangeProvider = {
  ...
  async diff(ctx) {
    ...
    for (const t of ctx.cfg.db.track_tables) {
      ...
      const { changed } = await deltaFor(ctx, dir, t, after);
      if (changed) cs.data[t] = changed;
    }
  ...
  async export(ctx, cs, dir) {
    ...
    const schema = await dumpSchema(ctx);
    for (const t of Object.keys(cs.data)) {
      const { upserts, removed } = await deltaFor(ctx, baseDir, t, schema);
