...
import { depHint, missingDeps } from "../core/deps.js";
...
export interface NewOpts { from?: string; install?: boolean; level?: number; task?: string; db?: DbStrategy; media?: MediaStrategy; noStart?: boolean; name?: string; pool?: boolean; noPool?: boolean }
...
export async function cmdNew(env: Env, branch: string, o: NewOpts) {
  return withRepoLock(env.repoRoot, async () => {
    ...
    if (poolEntry) {
      ...
      steps = planNew(ctx, env.adapters, env.providers, { from, start: !o.noStart, install: o.install });
    }
    ...
    const next = [
      ...
      env.opts.dryRun ? "" : (() => { const m = missingDeps(rec, env.adapters); return m.length ? depHint(rec, m) : ""; })(),
